            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
                    ...
                    <div className="space-y-4">
                        <Link to="/" className="flex items-center gap-3 mb-4">
                            <img
                                src={logo}
                                alt={import.meta.env.VITE_NGO_NAME + " Logo"}
                                className="h-12 w-auto object-contain brightness-0 invert"
                            />
                            <span className="font-heading font-bold text-lg tracking-tight leading-tight max-w-[200px]">
                                {import.meta.env.VITE_NGO_NAME}
                            </span>
                        </Link>
                        <p className="text-neutral-400 leading-relaxed">
                            Empowering Lives Through Education and Welfare.
                        </p>
                    ...
                    <div>
                        ...
                        <ul className="space-y-4">
                            <li className="flex items-start gap-3 text-neutral-400">
                                ...
                                <span className="break-words">{import.meta.env.VITE_ADDRESS}</span>
                            </li>
                            <li className="flex items-center gap-3 text-neutral-400">
                                <Phone className="w-5 h-5 text-primary-500 shrink-0" />
                                <a href={`tel:${import.meta.env.VITE_CONTACT_PHONE.split(',')[0].trim()}`} className="hover:text-white transition-colors">{import.meta.env.VITE_CONTACT_PHONE}</a>
                            </li>
                            <li className="flex items-center gap-3 text-neutral-400">
                                <Mail className="w-5 h-5 text-primary-500 shrink-0" />
                                <a href={`mailto:${import.meta.env.VITE_CONTACT_EMAIL}`} className="hover:text-white transition-colors">{import.meta.env.VITE_CONTACT_EMAIL}</a>
                            </li>
                        </ul>
                    </div>
                ...
                <div className="border-t border-neutral-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
                    <p className="text-neutral-500">
                        © {new Date().getFullYear()} {import.meta.env.VITE_NGO_NAME}. All rights reserved.
                    </p>
